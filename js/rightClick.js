 ...
 *   LoadImage       â†?"ðŸ” Describe with LLM" (file path â†?vision)
 *   IMAGE output    â†?"ðŸ” Describe with LLM" (cached output â†?vision)
 *   Text nodes      â†?"ðŸ“‹ Copy to Describe"
 */
...
async function describeImageNode(node) {
    ...
    if (!sidebar.selectedModel) {
        alert("Select a model in the ðŸ’¬ LLM sidebar first.");
        sidebar.togglePanel();
        return;
    }
